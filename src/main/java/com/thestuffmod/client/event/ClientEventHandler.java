...
public class ClientEventHandler {
	...
	public ClientEventHandler() {
		// Fail safe to prevent crashing if the array lengths don't match up (still crashes on length 0)
		int length = Math.min(desc.length, Math.min(category.length, keyValues.length));
		keys = new KeyBinding[length];
	...
	public void onTextureStitch(TextureStitchEvent.Post event) {
		/*
		 * The following line is commented out since I couldn't get it to work
		 */
		//event.map.setTextureEntry("tsm/canvas", new TextureCanvas("tsm/canvas"));
	}

	/*
	 * Render chat text above the appropriate player's head when they chat.
	 * The message is displayed for as 500 ms per character (makes longer messages stay up for longer)
	 * Not a good formula for reading speed
	 */
	@SubscribeEvent
	@SideOnly(Side.CLIENT)
	...
	/*
	 * Store received chat into the chat map of the correct player, and saves the message time-stamp using the current
	 * time millis.
	 */
	@SubscribeEvent
	@SideOnly(Side.CLIENT)
	...
	/*
	 * Intercept the item tool tips and add extra info if the language files have an 'item.ITEM_NAME.info' key
	 * 
	 * This won't happen by default, since the default keybinding is 'NONE', which disables it
	 */
	@SubscribeEvent
	@SideOnly(Side.CLIENT)
	...
	/*
	 * Hacky way of implementing drawn canvases showing in ItemFrames
	 * 
	 * Do not critique this.
	 */
	@SubscribeEvent
	@SideOnly(Side.CLIENT)
	...
	private void renderLabel(Entity entity, String string, double x, double y, double z, int distance) {
